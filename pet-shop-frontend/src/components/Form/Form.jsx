...
export default function Form() {
  ...
  const onSubmit = async (data) => {
    setIsLoading(true);
    try {
      const response = await axios.post('https://petshop-backend-33od.onrender.com/sale/send', data, {
        headers: {
  ...
  return (
    <>
      ...
      {showAlert && <div className={styles.alert}>Form submitted successfully!</div>}
    </>
  );
}
